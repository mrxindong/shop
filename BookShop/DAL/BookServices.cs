...
namespace BookShop.DAL
{
	...
	public partial class BookServices
	{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select top 15 Id,Title,Author,PublisherId,PublishDate,ISBN,WordsCount,UnitPrice,ContentDescription,AurhorDescription,EditorComment,TOC,CategoryId,Clicks ");
        ...
        /// <summary>
        /// 获得该分类下有多少本图书,如果categoryId=0,则获得共有多少本图书
        /// </summary>
        ...
        /// <summary>
        /// 在某分类下,根据起始行号和结束行号来获得相应的图书
        /// </summary>
        /// <param name="categoryId">图书分类id,当id传0时,为所有分类</param>
        /// <param name="start">开始的行号</param>
        /// <param name="end">结束的行号</param>
        /// <param name="orderby">排序依据 当传入null或""字串时,默认按id升序排序</param>
        /// <returns></returns>
        public DataSet GetListByNumber(int categoryId,int start,int end,string orderby)
        {

            string sql = "select * from "+
                  "(select (Row_number() over(order by {0})) as rownum,* from Books {1}) t"+
                  " where rownum between @start and @end";

            sql = string.Format(sql, (orderby == null || orderby == "") ? "id" : orderby,
                categoryId == 0 ? "" : " where categoryid=@cateid");
            ...
            SqlParameter[] parameters = {
                    ...
                    new SqlParameter("@cateid",categoryId)	
                                        };
