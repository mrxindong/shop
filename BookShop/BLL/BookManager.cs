...
namespace BookShop.BLL
{
	...
	public class BookManager
	{
        ...
        /// <summary>
        /// 获得当前分类下的图书共有几页
        /// pageSize每页的书的数量   categoryId分类id,如果为0,则为所有分类
        /// </summary>
        ...
        {
            //得到图书的数量
            int booksCount = dal.GetBookCountByCategoryId(categoryId);
            int pageCount=  Convert.ToInt32(Math.Ceiling( (booksCount*1.0) / pageSize));
            return pageCount;
        ...
        /// <param name="?"></param>
        /// <returns></returns>
        public List<Model.Book> GetBooksByPageNumber(int pageSize, int pageNumber, int categoryId,string orderby)
        {
            int start = (pageNumber - 1) * pageSize + 1;
            int end = pageNumber * pageSize;
            DataSet ds = dal.GetListByNumber(categoryId, start, end, orderby);
            return DataTableToList(ds.Tables[0]);

        }

        public List<Model.Book> GetBooksByPageNumber(int pageSize, int pageNumber, int categoryId)
        {
           return this.GetBooksByPageNumber(pageSize, pageNumber, categoryId, "");
        }

        public List<Model.Book> GetBooksByKey(string key)
        {
