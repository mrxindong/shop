...
namespace BookShop.BLL
{
	...
	public class CartManager
	{
		...
		/// <summary>
		/// 增加一条数据  根据返回值 true为修改数量   false表示在购物车中添加新的数据
		/// </summary>
		public bool  Add(BookShop.Model.Cart model)
		{
            //进行重复添加的判断
            Model.Cart oneCart = dal.GetModel(model.User.Id, model.Book.Id);
            if (oneCart == null)
            {
                //当前这个用户名下没有这本书,我们就添加一本
                 dal.Add(model);
                 return false;
            }
            else
        ...
        public void Update(int cartId,int bookCount)
        {
            dal.Update(cartId, bookCount);

        }

        public void Delete(int userId, int bookId)
        {
            dal.Delete(userId, bookId);
        }
		/// <summary>
		/// 获得数据列表
		/// </summary>
